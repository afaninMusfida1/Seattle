...
import { apiGetPresensiByTanggal, apiGetSiswaByIdKelas, addPresensi, updatePresensi, apiGetPresensi } from '../crud-presensi/requestPresensi';
...
const Jurnal = () => {
    ...
    const cekPresensi = async (kelas_id, tanggal) => {
        if (!tanggal) {
            Swal.fire({
                title: 'Perhatian',
                text: 'Mohon isi tanggal terlebih dahulu',
        ...
        const resultPresensi = await apiGetPresensiByTanggal(kelas_id, tanggal)

        if (!resultPresensi) {
            setJurnalIsAvailable(false);
            Swal.fire({
                title: 'Informasi',
                text: 'Belum ada Preseni, silahkan isi',
                icon: 'info',
                confirmButtonText: 'Oke'
            })
        } else {
            ...
            setKeterangan(resultPresensi.keterangan)
            Swal.fire({
                title: 'Informasi',
                text: 'Ditemukan presensi, silahkan update',
                icon: 'info',
                confirmButtonText: 'Lanjut'
            });
        };
    ...
    const handleUpdatePresensi = async (siswa_id, keterangan) => {
        ...
        try {
            await updatePresensi(idKbm, siswa_id, keterangan);
            console.log(`berhasil update presensi`);

        } catch (error) {
            console.log(`error updating presensi`, error);
            Swal.fire({
                title: 'Oops..',
                text: 'Gagal mengUpdate presensi',
                icon: 'error',
                    ...
                    <>
                        <div className='flex gap-6 relative'>
                            ...
                            <button onClick={async () => {
                                await checkJurnal(kelas_id, tanggal)
                                
                            }} className='bg-[#078DCC] rounded-md text-white px-3 py-2 active:opacity-50 outline-none'>
                                Cek Jurnal
                            </button>
                            <span
                    ...
                    <>
                        <div className="w-full max-h-[380px] overflow-auto flex flex-wrap gap-1">
                            {fetchedSiswaList.map(siswa => (
                                <PresensiItem key={siswa.id} siswa={siswa} tanggal={tanggal} idPresensi={idPresensi} handleUpdatePresensi={handleUpdatePresensi} handleAddPresensi={handleAddPresensi} jurnalIsAvailable={jurnalIsAvailable} />
                            ))}
                        </div>
                        <div className="flex justify-end">
                            <button className="bg-[#078DCC] text-white px-[70px] py-[5px] rounded mr-[20px] mt-[20px] self-end">
                                Selesai
                            </button>
                        </div>
                    </>
