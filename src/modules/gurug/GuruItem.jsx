...
const GuruItem = ({id, nama, hapusGuru}) => {
  ...
  if (isDeleted) {
    return null; // Jangan render apa-apa jika item dihapus
  }
    ...
    <div className="flex mt-[5px] ">
      <div className="flex flex-col mt-[20px] w-[180px] h-[220px] bg-[#FBFBFB] rounded-lg shadow-xl items-center space-y-1 py-3 relative">
        <div className="flex flex-col w-16 h-16 relative overflow-hidden rounded-full mt-[15px]">
          <img src="src/assets/imgGuru.jpg" className="w-full h-full rounded-full shadow-md" alt="Guru"/>
        </div>
        <div className="pt-[10px] text-center">
          <h2 className="text-2xl font-semibold text-gray-800">{nama}</h2>
        ...
        <button onClick={handleDelete} className='absolute top-2 right-3 text-[20px]'>
          <FontAwesomeIcon icon={faTrashCan} style={{color: "#6a6d76"}} />
        </button>
      </div>
    </div>
  );
